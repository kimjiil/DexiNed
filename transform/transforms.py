...
def intersect(box_a, box_b):
    ...
    return inter[:, 0] * inter[:, 1] # w * h => return intersection area
...
def jaccard_numpy(box_a, box_b):
    ...
    """
    inter = intersect(box_a, box_b) # inter 는 box_a , box_b의 intersection한 넓이들
...
class RandomGaussianBlur(object):
    ...
    def __call__(self, image, boxes=None, labels=None):
        kernel_size = 2*random.randint(0,15) + 1
        return cv2.GaussianBlur(image,(kernel_size,kernel_size),sigmaX=5) , boxes, labels
...
class RandomSampleCrop_OCRver(object):
    ...
    def __call__(self, image, boxes=None, labels=None):
        ...
        while True:
            ...
            for _ in range(50):
                ...
                # mask in all gt boxes that above and to the left of centers
                # ( rect_left < gt_box_center_left ) and ( rect_top < gt_box_center_top )
                m1 = (rect[0] < centers[:, 0]) * (rect[1] < centers[:, 1])

                # mask in all gt boxes that under and to the right of centers
                # ( rect_right < gt_box_center_right ) and ( rect_bottom < gt_box_center_bottom )
